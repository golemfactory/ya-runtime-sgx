...
impl Voting {
    ...
    pub fn save(&self) -> anyhow::Result<()> {
        let file_name = super::prv_path(Self::SAVED_FILE_NAME);
    ...
    pub fn load(
        ...
        let requested_contract = EthAddress::from_hex(requested_contract).with_context(|| format!("invalid contract {}", requested_contract))?;
        if requested_contract != contract {
            return Err(VotingError::InvalidAddress).context("requested contract different");
        }
        if *requested_voting_id != voting_id {
            return Err(VotingError::InvalidId.into());
        }
        let operator_addr = EthAddress::from_hex(operator_addr).with_context(|| format!("invalid operator address {}", operator_addr))?;
        if operator_addr != secret.to_eth_address() {
            return Err(VotingError::InvalidAddress).with_context(|| format!("diffrent operator {} != {}", operator_addr.to_hex_string(), secret.to_eth_address().to_hex_string()));
        }
    ...
    pub fn vote(&mut self, sender: &str, encrypted_vote: &str) -> anyhow::Result<String> {
        ...
        *self.results.entry(vote).or_insert(0) += 1;
        ...
        let encrypted_response = cipher
            ...
            .map_err(|e| anyhow::anyhow!("EncrpytionError"))?;
